...
public class PoseTracking : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (points.Length > 1)
        {
            ...
            float yOffset = Mathf.Abs(vrCamera.transform.position.y - leftEyeYPosition);
            float xOffset = vrCamera.transform.position.x;

            float zPosition = vrCamera.transform.position.z;
